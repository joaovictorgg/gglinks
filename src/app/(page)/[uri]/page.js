...
import { faEnvelope, faLink, faMapPin, faMobile } from "@fortawesome/free-solid-svg-icons";
...
export default async function PaginaDoUsuario({params}) {
    const uri = params.uri;
    mongoose.connect(process.env.MONGODB_URI);
    const page = await Page.findOne({uri});
    const user = await User.findOne({email:page.owner});
    await Eventos.create({uri:uri, page:uri, type:'view'});
    return(
        <div className="bg-green-800 text-white min-h-screen">
            <div className="h-36 bg-gray-800 bg-cover bg-center"
            style={
              page.bgType === 'color'
            ...
            <div className="aspect-square w-36 h-36 mx-auto relative -top-16
            -mb-12">
                <Image 
            ...
            text-white/70">
                <FontAwesomeIcon className="h-4" icon={faMapPin} />
                <span>
                  {page.location}
                </span>
            </h3>
            ...
            <div className="flex gap-2 justify-center mt-4 pb-4">
                {Object.keys(page.buttons).map(buttonKey => (
                    <Link key={buttonKey} href={buttonLink(buttonKey, page.buttons[buttonKey])} 
                        className="rounded-full bg-white text-green-800 p-2
            ...
            <div className="max-w-2xl mx-auto grid md:grid-cols-2 gap-6 p-4
                ...
                {page.links.map(link => (
                    ...
                    className="bg-green-900 p-2 block flex"
                    href={link.url}>
                        <div className="relative -left-4  overflow-hidden w-16 ">
                            <div className="w-16 h-16 bg-green-700 aspect-square relative
                                    flex items-center justify-center ">
                                {link.icon && (
                                    <Image 
